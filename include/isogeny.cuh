 ...
 * @param P2 Point with exact order 2 (mont_pt_t: montgomery.h)
 * @param P Point to be lifted to the 2-isogeny (mont_pt_t: montgomery.h)
 * @param isoP Point lifted to the 2-isogeny (mont_pt_t: montgomery.h)
 */
template<typename env_t>
__host__ __device__ void 
eval_2_iso(env_t env,
    const typename env_t::cgbn_t& p, 
    const mont_pt_t<env_t>& P2, 
    const mont_pt_t<env_t>& P, 
    mont_pt_t<env_t>& isoP) {
  return;
#if 0        
  const fp2* x2 = &P2->x;
  const fp2* x = &P->x;
  ...
  fp2_Multiply(p, x, x2, &t1);         // t1:=x*x2;  // t1 = x*x2
  fp2_Multiply(p, x, &t1, &t2);        // t2:=x*t1;  // t2 = x^2*x2
  fp2_Multiply(p, &t1, x2, &t3);       // t3:=t1*x2; // t3 = x*x2^2
  fp2_Add(p, &t3, &t3, &t3);           // t3:=t3+t3; // t3 = 2*x*x2^2
  fp2_Sub(p, &t2, &t3, &t3);           // t3:=t2-t3; // t3 = x^2*x2-2*x*x2^2
  fp2_Add(p, &t3, x2, &t3);            // t3:=t3+x2; // t3 = x^2*x2-2*x*x2^2+x2
  fp2_Multiply(p, y, &t3, &t3);        // t3:=y*t3;  // t3 = y*(x^2*x2-2*x*x2^2+x2)
  fp2_Sub(p, &t2, x, &t2);             // t2:=t2-x;  // t2 = x^2*x2-x
  fp2_Sub(p, x, x2, &t1);              // t1:=x-x2;  // t1 = x-x2
  fp2_Invert(p, &t1, &t1);             // t1:=1/t1;  // t1 = 1/(x-x2)
  fp2_Multiply(p, &t2, &t1, xx);       // xx:=t2*t1; // xx = (x^2*x2-x)/(x-x2)
  fp2_Square(p, &t1, &t1);             // t1:=t1^2;  // t1 = 1/(x-x2)^2
  fp2_Multiply(p, &t3, &t1, yy);       // yy:=t3*t1; // yy = y*(x^2*x2-2*x*x2^2+x2)/(x-x2)^2
 ...
 *
 * @param p Finite field parameters (ff_Params: fp.h)
 * @param P2 Point with exact order 2 on E (mont_pt_t: montgomery.h)
 * @param E Curve to be lifted to the 2-isogeny (mont_curve_int_t: montgomery.h)
 * @param isoE 2-isogenous curve to E (mont_curve_int_t: montgomery.h)
 */
template<typename env_t>
__host__ __device__ void 
curve_2_iso(env_t env,
    const typename env_t::cgbn_t& p, 
    const mont_pt_t<env_t>& P2, 
    const mont_curve_int_t<env_t>& E, 
    mont_curve_int_t<env_t>& isoE) {
  return;
#if 0        
  // aa:=2*(1-2*x2^2);
  // bb:=x2*b;
  ...
  fp2_Square(p, x2, &t1);     // t1 = x2^2
  fp2_Add(p, &t1, &t1, &t1);  // t1 = 2*x2^2
  fp2_Sub(p, &one, &t1, &t1); // t1 = 1-2*x2^2
  fp2_Add(p, &t1, &t1, aa);   // aa = 2*(1-2*x2^2)
  fp2_Multiply(p, x2, b, bb); // bb = x2*b
 ...
 *
 * @param p Finite field parameters (ff_Params: fp.h)
 * @param P3 Point with exact order 3 (mont_pt_t: montgomery.h)
 * @param P Point to be lifted to the 3-isogeny (mont_pt_t: montgomery.h)
 * @param isoP Point lifted to the 3-isogeny (mont_pt_t: montgomery.h)
 */
template<typename env_t>
__host__ __device__ void 
eval_3_iso(env_t env,
    const typename env_t::cgbn_t& p, 
    const mont_pt_t<env_t>& P3, 
    const mont_pt_t<env_t>& P, 
    mont_pt_t<env_t>& isoP) {
      return;
    }
 ...
 *
 * @param p Finite field parameters (ff_Params: fp.h)
 * @param P3 Point with exact order 3 on E (mont_pt_t: montgomery.h)
 * @param E Curve to be lifted to the 3-isogeny (mont_curve_int_t: montgomery.h)
 * @param isoE 3-isogenous curve to E (mont_curve_int_t: montgomery.h)
 */
template<typename env_t>
__host__ __device__ void 
curve_3_iso(env_t env,
    const typename env_t::cgbn_t& p, 
    const mont_pt_t<env_t>& P3, 
    const mont_curve_int_t<env_t>& E, 
    mont_curve_int_t<env_t>& isoE) {
 ...
 *
 * @param p Finite field parameters (ff_Params: fp.h)
 * @param P4 Point with exact order 4 (mont_pt_t: montgomery.h)
 * @param P Point to be lifted to the 4-isogeny (mont_pt_t: montgomery.h)
 * @param isoP Point lifted to the 4-isogeny (mont_pt_t: montgomery.h)
 */
template<typename env_t>
__host__ __device__ void 
eval_4_iso(env_t env,
    const typename env_t::cgbn_t& p, 
    const mont_pt_t<env_t>& P4, 
    const mont_pt_t<env_t>& P, 
    mont_pt_t<env_t>& isoP) {
  return;
#if 0        
 ...
 *
 * @param p Finite field parameters (ff_Params: fp.h)
 * @param P4 Point with exact order 4 on E (mont_pt_t: montgomery.h)
 * @param E Curve to be lifted to the 4-isogeny (mont_curve_int_t: montgomery.h)
 * @param isoE 4-isogenous curve to E (mont_curve_int_t: montgomery.h)
 */
template<typename env_t>
__host__ __device__ void 
curve_4_iso(env_t env,
    const typename env_t::cgbn_t& p, 
    const mont_pt_t<env_t>& P4, 
    const mont_curve_int_t<env_t>& E, 
    mont_curve_int_t<env_t>& isoE) {
 ...
 * 
 * @param p Finite field parameters (ff_Params: fp.h)
 * @param e e2 from public parameters (int32_t)
 * @param E E_{a,b} (mont_curve_int_t: montgomery.h)
 * @param S S with exact order 2^e2 on E. May be modified internally. (mont_pt_t: montgomery.h)
 * @param P1 P1 on E (may be NULL) (mont_pt_t: montgomery.h)
 * @param P2 P2 on E (may be NULL) (mont_pt_t: montgomery.h)
 * @param isoE E_{a',b') (mont_curve_int_t: montgomery.h)
 * @param isoP1 P1 lifted to isoE (mont_pt_t: montgomery.h)
 * @param isoP2 P2 lifted to isoE (mont_pt_t: montgomery.h)
 */
template<typename env_t>
__host__ __device__ void 
iso_2_e(env_t env,
    ...
    const mont_curve_int_t<env_t>& E, 
    mont_pt_t<env_t>& S,
    const mont_pt_t<env_t>& P1, 
    const mont_pt_t<env_t>& P2,
    mont_curve_int_t<env_t>& isoE,
    mont_pt_t<env_t>& isoP1,
    mont_pt_t<env_t>& isoP2) {
  return;
#if 0
  ...
  if (e % 2) {
    xDBLe(E, S, e - 1, &T);
    curve_2_iso(p, &T, isoE, isoE);
    eval_2_iso(p, &T, S, S);

    if (p1Eval)
      eval_2_iso(p, &T, isoP1, isoP1);

    if (p2Eval)
      eval_2_iso(p, &T, isoP2, isoP2);
 ...
 * 
 * @param p Finite field parameters (ff_Params: fp.h)
 * @param e e3 from public parameters (int32_t)
 * @param E E_{a,b} (mont_curve_int_t: montgomery.h)
 * @param S S with exact order 2^e3 on E. May be modified internally. (mont_pt_t: montgomery.h)
 * @param P1 P1 on E (may be NULL) (mont_pt_t: montgomery.h)
 * @param P2 P2 on E (may be NULL) (mont_pt_t: montgomery.h)
 * @param isoE E_{a',b') (mont_curve_int_t: montgomery.h)
 * @param isoP1 P1 lifted to isoE (mont_pt_t: montgomery.h)
